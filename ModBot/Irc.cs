...
namespace ModBot
{
    class Irc
    {
        ...
        public Irc(String nick, String password, String channel, String currency, int interval, int payout) {
            ...
            setPayout(payout);
            offlineHandouts = ModBot.Properties.Settings.Default.offlineHandouts;
        ...
        private void Connect()
        {
            ...
            while (!irc.Connected)
            {
                Console.WriteLine("Connect attempt " + connectRetry);
                try
                {
                    //irc.Connect("199.9.250.239", 80);
                    ...
                    sendRaw("NICK " + nick.ToLower());
                    sendRaw("USER " + nick.ToLower() + " 8 * :" + nick.ToLower());
            ...
            if (!Connected())
            {                
                Console.WriteLine("Connection refused.  Check your username and password.  Retrying.");
                Connect();
            }
            else
            {
        ...
        private void Listen()
        {
            ...
            {
                Console.WriteLine("Uh oh, major error.  Try restarting the bot.  If you keep seeing this message, email your Error_log.log file to twitch.tv.modbot@gmail.com");
                StreamWriter errorLog = new StreamWriter("Error_Log.log", true);
        ...
        private void handleMessage(String message)
        {
            ...
            if (msg[0].Equals("!raffle") && msg.Length >= 2) {
                ...
                if (db.getUserLevel(user) >= 1) 
                {
                    ...
                    else if (msg[1].Equals("close"))
                    {
                        ...
                        else
                        {
                            ...
                            if (msg.Length > 2 && int.TryParse(msg[2], out numWinners) && numWinners > 0 && numWinners <= raffle.getTotalTicketsPurchased())
                            {
                                if (numWinners <= raffle.maxDraw())
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        for (int i = 0; i < numWinners; i++)
                                        {
                                            ...
                                            winners.Append(checkBtag(raffle.getWinner()) + checkFollowing(raffle.getWinner()) + "!! (" + raffle.getPersonalTicketsPurchased(raffle.getWinner()) + " tickets purchased)");
                                            addComma = true;
                                        }
                                ...
                                {
                                    sendMessage("There are only " + raffle.maxDraw() + " people in the raffle who haven't won yet.  Your amount of winners to draw must be less than or equal to " + raffle.maxDraw(), 1);
                                }
                            }
                            else
                            {                                   
                                ...
                                else
                                {
                                    ...
                                    raffle.endRaffle();
                                    sendMessage(checkBtag(raffle.getWinner()) + checkFollowing(raffle.getWinner()) + "!! (" + raffle.getPersonalTicketsPurchased(raffle.getWinner()) + " tickets purchased)", 2);
                                }
                            }
                    ...
                    else if (msg[1].Equals("entries"))
                    {
                        if (raffleOpen)
                        {
                            sendMessage("Total entrants: " + raffle.maxDraw() + " - Tickets Purchased: " + raffle.getTotalTicketsPurchased(), 2);
                        }
                    }
                }
                ...
                else if (msg.Length >= 3)
                {
                    ...
                    if (msg[1].Equals("add") && db.getUserLevel(user) >= 2)
                    {
                        ...
                        if (int.TryParse(msg[2], out amount) && msg.Length >= 4)
                        {
                            ...
                            {
                                if (db.addCurrency(capName(msg[3]), amount))
                                {
                                ...
                                {
                                    sendMessage(capName(msg[3]) + " does not exists in the database.  Make sure you typed their name correctly! If you know you typed their name right, have them type !<currency> to get added to the db, then try again.", 1);
                                }
                            }
                        }
                    }
                    ...
                    if (msg[1].Equals("remove") && db.getUserLevel(user) >= 2)
                    {
                        ...
                        if (msg[2] != null && int.TryParse(msg[2], out amount) && msg.Length > -4)
                        {
                            ...
                            {
                                if (db.removeCurrency(capName(msg[3]), amount))
                                {
            ...
            else if (msg[0].Equals("!admin") && db.getUserLevel(user) >= 3 && msg.Length >= 2)
            {
                ...
                if (msg[1].Equals("offline") && msg.Length >= 3)
                {
                    if (msg[2].Equals("enabled"))
                    {
                        offlineHandouts = true;
                        ModBot.Properties.Settings.Default.offlineHandouts = true;
                        ModBot.Properties.Settings.Default.Save();
                        sendMessage("Enabled handouts when the stream is offline.", 2);
                    }
                    if (msg[2].Equals("disabled"))
                    {
                        offlineHandouts = false;
                        ModBot.Properties.Settings.Default.offlineHandouts = false;
                        ModBot.Properties.Settings.Default.Save();
                        sendMessage("Disabled handouts when the stream is offline.", 2);
                    }
                }
            ...
            ///////////////CUSTOM COMMANDS//////////////////////////
            #region custom
            else if (commands.cmdExists(msg[0].ToLower()) && db.getUserLevel(user) >= commands.LevelRequired(msg[0]))
            {
            ...
            ////////////////EXTRA COMMANDS///////////////////////////
            else if ((msg[0].Equals("!btag")|| msg[0].Equals("!battletag")) && msg.Length == 2 && msg[1].Contains("#"))
            {
           ...
           /* if (msg[0].Equals("!restart") && db.getUserLevel(user) == 3)
            ...
            else if (msg[0].Equals("!test") && user.Equals("keirathi", StringComparison.OrdinalIgnoreCase))
            {
                String[] tempUsers = new String[int.Parse(msg[1])];
                for (int i = 0; i < int.Parse(msg[1]); i++)
                {
                    tempUsers[i] = "test" + i;
                }
                db.addCurrency(tempUsers, int.Parse(msg[2]));
            }

            else if (msg[0].Equals("!force") && user.Equals("keirathi", StringComparison.OrdinalIgnoreCase))
            {
                handoutCurrency();
            }

            else if (msg[0].Equals("!top") && db.getUserLevel(user) == 3)
            {
                int amount;
                if (msg.Length >= 2 && int.TryParse(msg[1], out amount) && amount > 0 && amount <= db.totalUsers())
                {
                    string[] topList = db.getTop(amount);
                    StringBuilder sb = new StringBuilder();
                    sb.Append("Current users with the most " + currency + ": ");
                    bool addComma = false;
                    for (int i = 0; i < topList.Length; i++)
                    {
                        ...
                        sb.Append(i + 1 + ". " + topList[i] + " (" + db.checkCurrency(topList[i]) + ")");
                        addComma = true;
                    }
        ...
        private string checkFollowing(String user)
        {
            using (var w = new WebClient())
            {
                ...
                {
                    w.Proxy = null;
                    json_data = w.DownloadString("https://api.twitch.tv/kraken/users/" + user + "/follows/channels/" + channel.Substring(1));
                    //Console.WriteLine(json_data);
                    return "*";
                }
                catch (SocketException e)
                {
                    Console.WriteLine("Problem connecting to twitch API.  Skipping following check.");
                    return "";
                }
        ...
        private void handoutCurrency()
        {
            try
            {
                buildUserList();
                checkChatList();
                String subs = checkSubList();
                Thread.Sleep(5000);
                //Console.WriteLine(temp);
                List<String> subList = new List<String>();
                string[] tUsers = new string[users.Count];
                users.CopyTo(tUsers);
                lock (tUsers)
                {
                    foreach (String person in tUsers)
                    {
                        //Console.WriteLine(person);
                        if (db.isSubscriber(person) || subs.Contains(person))
                        {
                            subList.Add(person);
                            //users.Remove(person);
                        }
                    ...
                    db.addCurrency(tUsers, payout);
                    db.addCurrency(subList.ToArray(), payout);
                }
            }
            catch (Exception e)
            {
        ...
        {
            string apiEndpoint = "https://tmi.twitch.tv/group/user/" + admin + "/chatters";
            ...
            using (var w = new WebClient())
            {
                ...
                {
                    w.Proxy = null;
                    string json = w.DownloadString(apiEndpoint);
                    JObject list = JObject.Parse(json);
                    foreach (JProperty mode in list["chatters"])
                    {
                        foreach (var user in list["chatters"][mode.Name])
                        {
                            addUserToList(user.Value<string>());
                        }
                    }
                }
                catch (Exception e)
            ...
            while (true)
            {
                try
                {
                    if (checkTime())
                    {
                        ...
                        else if (offlineHandouts)
                        {
                            print("Offline handout happening now!  Paying everyone " + payout + " " + currency);
                            handoutCurrency();
                        }
                    }
                    Thread.Sleep(60000);
